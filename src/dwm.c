...
enum { SchemeNorm, SchemeSel, SchemeSelTitle, SchemeSystray, SchemeBorder }; /* color schemes */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		} else if (ev->x > selmon->ww - ((showsystray && !systrayonleft && selmon == systraytomon(selmon)) ? (int)getsystraywidth() : 0) - statusw - 2 * sp) {
			int stw = (showsystray && !systrayonleft && selmon == systraytomon(selmon)) ? (int)getsystraywidth() : 0;
			x = selmon->ww - stw - statusw - 2 * sp;
			click = ClkStatusText;
			statussig = 0;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	wc.border_width = c->bw;
	if (nobrodersingle && (((nexttiled(c->mon->clients) == c && !nexttiled(c->next))
